...
class CommonService {
  ...
  findAll = async (filter = {}, trx) => {
    const { queryMeta } = ___.omitBy(filter, ___.isNull);
    const pMeta = await this.paginate.makeMeta(queryMeta);
    ...
    return {
      ...(queryMeta?.columns && {columns: Object.keys(await this.model.describe())}),
      ...((queryMeta?.paginate && { meta: { ...pMeta, count } }) || {}),
      data: ___.map(res, util.toJSON),
    };
  };

  findOne = async (filter = {}) => {
    const model =
      (filter?.scopes && this.model.scope(filter.scopes)) || this.model;
    const options = await this.makeOptions(filter);
    options.limit = 1;
    return await model.findOne(options);
  };

  update = async (filter, inputDto, txn) => {
    await this.model.update(___.omit(inputDto, ['include']), {
      where: filter,
      ...___.pick(inputDto, ['include'])
    });
  ...
  upsert = async (filter, inputDto, txn) => {
  ...
  makeIncludeOptions = async (includeMeta) => {
    return (includeMeta || []).map((val) => {
      val.where &&
        (val.where = this.paginate.filter({ filterMeta: val.where }));
  ...
  makeOptions = async (filter) => {
    const { queryMeta, filterMeta, attributeMeta, includeMeta, ...filters } =
      ___.omitBy(filter, ___.isNull);
    const pFilter = await this.paginate.calcPage(queryMeta);
    const pInclude = await this.makeIncludeOptions(
      ___.filter(includeMeta, (val) => val)
    );
